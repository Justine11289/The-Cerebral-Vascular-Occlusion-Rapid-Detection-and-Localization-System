...
parser.add_argument('--h5_data_dir', default='hdf5_data', help='input hdf5 dir')
parser.add_argument('--test_dir', default='PartAnnotation', help='input hdf5 test data dir')
parser.add_argument('--out_dir', default='test_results', help='output dir')
FLAGS = parser.parse_args()


# DEFAULT SETTINGS
pretrained_model_path = FLAGS.model_path # os.path.join(BASE_DIR, './pretrained_model/model.ckpt')
hdf5_data_dir = os.path.join(BASE_DIR, './data/'+FLAGS.h5_data_dir)
ply_data_dir = os.path.join(BASE_DIR, './'+FLAGS.test_dir)
gpu_to_use = 0
output_dir = os.path.join(BASE_DIR, './'+FLAGS.out_dir)
output_verbose = True   # If true, output all color-coded part segmentation obj files

# MAIN SCRIPT
point_num = 8000            # the max number of points in the all testing data shapes
batch_size = 1
...
def predict():
    ...
    with tf.Session(config=config) as sess:
        ...
        for shape_idx in range(len_pts_files):
            ...
            #print seg_pred_val
            '''for i in range(len(seg_pred_val)):
                print seg_pred_val[i]'''
